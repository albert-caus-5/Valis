...
class NonRigidZImage(object):
    ...
    #Comentari: He afegit el mask_dir per tal de poder guardar imatges per entendre el codi
    def calc_deformation(self, mask_dir_non_rigid_registration, mask_dir_non_rigid_non_rigid_registrars, registered_fixed_image, non_rigid_reg_class,
        ...
        warped_moving, moving_grid_img, moving_bk_dxdy = \
            non_rigid_reg.register(mask_dir_non_rigid_registration = mask_dir_non_rigid_registration, 
                                   mask_dir_non_rigid_non_rigid_registrars=mask_dir_non_rigid_non_rigid_registrars, 
                                   moving_img=moving_img,
                                   fixed_img=registered_fixed_image,
                                   mask=reg_mask,
                                   **reg_kwargs)
            #Comentari: He afegit el mask_dir per tal de poder guardar diferents imatges per tal d'entendre el codi
            #Aquí crida el mètode register de la superclasse NonRigidRegistrar del non_rigid_registrars.py

        #Codi meu
        #print(f'La moving img que se li passa al non_rigid_registrars.py és: {moving_img.size}')
        #print(f'La fixed img que se li passa al non_rigid_registrars.py és: {fixed_img.size}')
        #Tot té dimensions (P,Q), que són les dimensions de la màscara que és de 1850x1838
        ...
        #Codi meu
        #mask1_path = os.path.join(mask_dir_non_rigid_registration, "0_Slide_mask_6.png")
        #mask1 = cv2.imread(mask1_path, cv2.IMREAD_GRAYSCALE)

        #print(f'Mida bk_dxdy_from_ref: {bk_dxdy_from_ref.shape}')
...
class SerialNonRigidRegistrar(object):
    ...
    def register_serial(self, mask_dir_non_rigid_registration, mask_dir_non_rigid_serial_non_rigid, mask_dir_non_rigid_non_rigid_registrars, non_rigid_reg_class, non_rigid_reg_params=None, img_params=None):
        ...
        for moving_idx, fixed_idx in tqdm(iter_order, desc="Finding non-rigid transforms", unit="image"):
            ...
            #Codi meu
            #print(f'Moving object {self.non_rigid_obj_list[moving_idx].name}')
            #print(f'Fixed object {self.non_rigid_obj_list[fixed_idx].name}')
            #Printeja això:
                #Moving object AR22B050886B001002
                #Fixed object AR22B050886B001010
            #
            ...
            if moving_obj.mask is not None:
                if self.mask is not None:
                    ...
                    #Codi meu
                    warp_tools.save_img(os.path.join(mask_dir_non_rigid_serial_non_rigid, f'mascara_moving_obj_non_rigid.png'), moving_obj.mask)
                    #Guarda una màscara amb tot blanc de 1850x1838
                    #Codi meu
                    #print(f'4')
                    #Entra aquí
            ...
            updated_dxdy = moving_obj.calc_deformation(mask_dir_non_rigid_registration = mask_dir_non_rigid_registration, 
                                                       mask_dir_non_rigid_non_rigid_registrars = mask_dir_non_rigid_non_rigid_registrars, registered_fixed_image=fixed_obj.registered_img,
                                        non_rigid_reg_class=non_rigid_reg_class,
                                        bk_dxdy=current_dxdy,
                                        params=nr_reg_params,
                                        mask=reg_mask
                                        )
            #Comentari: He afegit el mask_dir per tal de poder guardar les diferents imatges per entendre el codi
    ...
    def register(self, mask_dir_non_rigid_registration, mask_dir_non_rigid_serial_non_rigid, mask_dir_non_rigid_non_rigid_registrars, non_rigid_reg_class, non_rigid_reg_params, img_params=None):
        ...
        if issubclass(non_rigid_reg_class, non_rigid_registrars.NonRigidRegistrarGroupwise):
            ...
            #Codi meu
            #print(f'O aquest altre')
            #Entra aquí
            self.register_serial(mask_dir_non_rigid_registration = mask_dir_non_rigid_registration, mask_dir_non_rigid_serial_non_rigid = mask_dir_non_rigid_serial_non_rigid, mask_dir_non_rigid_non_rigid_registrars = mask_dir_non_rigid_non_rigid_registrars, non_rigid_reg_class = non_rigid_reg_class, non_rigid_reg_params = non_rigid_reg_params, img_params=named_img_params)
            #Comentari: He afegit el mask_dir per tal de poder guardar les diferents imatges per poder entendre el codi                                 non_rigid_reg_class, non_rigid_reg_params=None, img_params=
...
def register_images(src, mask_dir_non_rigid_registration, mask_dir_non_rigid_serial_non_rigid, mask_dir_non_rigid_non_rigid_registrars, non_rigid_reg_class=non_rigid_registrars.OpticalFlowWarper,
    ...
    nr_reg.register(mask_dir_non_rigid_registration = mask_dir_non_rigid_registration, mask_dir_non_rigid_serial_non_rigid = mask_dir_non_rigid_serial_non_rigid, mask_dir_non_rigid_non_rigid_registrars=mask_dir_non_rigid_non_rigid_registrars, non_rigid_reg_class = non_rigid_reg_class, non_rigid_reg_params = non_rigid_reg_params, img_params=img_params)

    if dst_dir is not None:
        #Codi meu
        #print(f'Entra dins dst_dir al serial non_rigid register_images')
        #No entra
