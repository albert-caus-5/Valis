...
class NonRigidRegistrar(object):
    ...
    #Comentari: He afegit el mask_dir per tal de poder guardar imatges a l'hora d'entendre el codi
    def register(self, mask_dir_non_rigid_registration, mask_dir_non_rigid_non_rigid_registrars, moving_img, fixed_img, mask=None, **kwargs):
        """
        ...
        **kwargs : dict, optional
        ...
        if self.mask is not None:
            ...
            #Obs: La màscara és una de [1850,1838] i és tota blanca

            # Only do registration inside mask #
            _, masked_fixed = self.apply_mask(self.mask)
            #La masked_fixed és la imatge que la Slide_mask_5 del prep_images_for_large_non_rigid_registration però sutilment més brillant
            masked_moving = self.moving_img.copy()
            #Entenc que és com la masked_fixed, és a dir, que és el resultat d'aplicar una equalització, però a la imatge d'HE, no la IHC
            #Comentari: Aquestes dos imatges són les quines es faran servir per calcular el camp de moviment
            ...
            #Codi meu
            path_mask = os.path.join(mask_dir_non_rigid_non_rigid_registrars, "Masked_fixed_non_rigid_registrars.png")
            warp_tools.save_img(path_mask, masked_fixed)
            path_mask_2 = os.path.join(mask_dir_non_rigid_non_rigid_registrars, "Masked_moving_non_rigid_registrars.png")
            warp_tools.save_img(path_mask_2, masked_moving)
            #
        ...
        #Càlcul del camp de desplaçament
        ...
        if mask is not None:
            ...
            print(f'Mida de la matriu bk_dxdy: {self.shape}')
        ...
        warped_img, warp_grid = self.get_warped_img_and_grid(mask_dir_non_rigid_registration, bk_dxdy)


        mask_dir_non_rigid_registration

        #Codi meu
        path_mask_3 = os.path.join(mask_dir_non_rigid_non_rigid_registrars, "warped_img_non_rigid_registrars.png")
        warp_tools.save_img(path_mask_3, warped_img)
        path_mask_4 = os.path.join(mask_dir_non_rigid_non_rigid_registrars, "warp_grid_non_rigid_registrars.png")
        warp_tools.save_img(path_mask_4, warp_grid)
        #
    ...
    def get_warped_img_and_grid(self, mask_dir_non_rigid_registration, bk_dxdy):
        ...
        warped_img_1 =transform.warp(mask1, warp_map, preserve_range=True)
        mask1_path_1 = os.path.join(mask_dir_non_rigid_registration, "0_Slide_mask_6_non_registered.png")
        warp_tools.save_img(mask1_path_1, warped_img_1)

        mask2_path = os.path.join(mask_dir_non_rigid_registration, "0_Slide_mask_4.png")
        mask2 = cv2.imread(mask2_path, cv2.IMREAD_GRAYSCALE)
        warped_img_2 = transform.warp(mask2, warp_map, preserve_range=True)
        mask2_path_2 = os.path.join(mask_dir_non_rigid_registration, "0_Slide_mask_4_non_registered.png")
        warp_tools.save_img(mask2_path_2, warped_img_2)

        mask3_path = os.path.join(mask_dir_non_rigid_registration, "HE_etiquetes_Valis_non_registering.png")
        mask3 =imread(mask3_path)
        warped_img_3 = transform.warp(mask3, warp_map, preserve_range=True)
        mask3_path_3 = os.path.join(mask_dir_non_rigid_registration, "HE_etiquetes_Valis_non_registered.png")
        warp_tools.save_img(mask3_path_3, warped_img_3)
        #
        self.warped_image = warped_img
