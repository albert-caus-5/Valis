...
class SerialRigidRegistrar(object):
    ...
    def generate_img_obj_list(self, feature_detector, qt_emitter=None):
        ...
        #Codi meu
        #print(f"Dimensions del SerialRigidRegistrar al serial_rigid: {out_w}, {out_h}")
        #
        ...
        #Codi meu
        print(f"Mida de la imatge al entrar al serial_rigid: {out_w}, {out_h}")
        print(f"Mida de out_shape (després d'aplicar la matriu T) al serial_rigid: {out_shape}")
        #
        ...
        for i in tqdm(range(self.size), desc=FEATURE_MSG, unit="image", leave=None):
            ...
            #Codi meu
            print(f"Valors de la Matriu T al serial_rigid: {img_obj.T}")
            print
            #
    ...
    def align_to_prev_check_reflections(self, transformer, feature_detector, matcher_obj, keep_unfiltered=False, qt_emitter=None):
        ...
        for moving_idx, fixed_idx in tqdm(self.iter_order, desc=TRANSFORM_MSG, unit="image", leave=None):
            ...
            #Codi meu
            print(f"Matriu reflection_M: {img_obj.reflection_M}")
            #
    ...
    def align_to_prev(self, transformer, qt_emitter=None):
        ...
        for moving_idx, fixed_idx in tqdm(self.iter_order, desc=TRANSFORM_MSG, unit="image", leave=None):
            ...
            #Codi meu
            print(f"Matriu to_prev_A: {img_obj.to_prev_A}")
            #
    ...
    def optimize(self, affine_optimizer, qt_emitter=None):
        ...
        for moving_idx, fixed_idx in tqdm(self.iter_order, desc=OPTIMIZING_MSG, unit="image", leave=None):
            ...
            #Codi meu
            print(f"Matriu optimal_M: {img_obj.optimal_M}")
            print(f"Mida de la imatge després d'aplicar la optimal_M: {prev_img}")
            #
            if qt_emitter is not None:
                qt_emitter.emit(1)
    ...
    def calc_warped_img_size(self):
        ...
        #print(f"Mesures del serial_rigid {w}, {h}")
        return np.array([h, w])

    def finalize(self, mask_dir_rigid_registration, mask_dir_rigid_serial_rigid):
        ...
        for i, img_obj in enumerate(self.img_obj_list):
            ...
            print(f'Mida de les dimensions: {img_obj.registered_shape_rc}')
            dimensions_girades = img_obj.registered_shape_rc[::-1]
            print(f'Mida de les dimensions girades: {dimensions_girades}')
            #Codi meu:
            
            if i==0:
                mask_path = os.path.join(mask_dir_rigid_registration, "Màscara_teixit_HE_original.png")
                img = cv2.imread(mask_path)
                # Matrius de transformació

                # Funció per aplicar la matriu i guardar la imatge
                transformed = cv2.warpPerspective(img, img_obj.M_inv, dimensions_girades) #Pot ser que s'hagi de girar l'ordre de les dimensions, que per contres de ser (x,y) sigui de la mida (y,x)
                warp_tools.save_img(os.path.join(mask_dir_rigid_serial_rigid, "output_M_inv.png"), transformed)


                mask_path_2 = os.path.join(mask_dir_rigid_serial_rigid, "Imatge_teixit_HE_retallada.png")
                img_2 = cv2.imread(mask_path_2, cv2.IMREAD_GRAYSCALE)
                #transformed_2_1 = 255 - img_2
                #cv2.imwrite(os.path.join(mask_dir_rigid_serial_rigid, "output_M_inv_1.png"), transformed_2_1)
                transformed_2 = cv2.warpPerspective(img_2, img_obj.M_inv, dimensions_girades) #Pot ser que s'hagi de girar l'ordre de les dimensions, que per contres de ser (x,y) sigui de la mida (y,x)
                warp_tools.save_img(os.path.join(mask_dir_rigid_serial_rigid, "output_M_inv_2.png"), transformed_2)

                #print(f"Matriu crop_T: {img_obj.crop_T}")
                #print(f"Matriu M: {img_obj.M}")
                #print(f"Mida de la imatge registrada final: {img_obj.registered_img.shape}")

                mask_path_3 = os.path.join(mask_dir_rigid_registration, "HE_etiquetes_Valis.png")
                img_3 = cv2.imread(mask_path_3)
                transformed_3 = cv2.warpPerspective(img_3, img_obj.M_inv, dimensions_girades) #Pot ser que s'hagi de girar l'ordre de les dimensions, que per contres de ser (x,y) sigui de la mida (y,x)
                warp_tools.save_img(os.path.join(mask_dir_rigid_serial_rigid, "output_M_inv_3.png"), transformed_3)
...
#Codi meu: He afegit la variable mask_dir, el pq està explicat al registration.py, rigid_registrar quan crido a aquesta funció
def register_images(mask_dir_rigid_registration, mask_dir_rigid_serial_rigid, img_dir, dst_dir=None, name="registrar",
    ...
    if valis_obj is not None:
        if valis_obj.create_masks:
            ...
            for img_obj in registrar.img_obj_dict.values():
                ...
                #Fer aquí la multiplicació de la HE amb la bbox amb M_inv aplicada (les imatges de la bbox i la HE són de la mateixa mida)
                #Aquí s'ha d'aplicar la matriu M_inv a la bbox (La imatge de la bbox no té la mateixa mida que la M_inv)
                #Hauria de ficar una variable global per saber a quina iteració estic i poder-hi accedir tant en el rígid com en el non-rígid

                #He de borrar les variables o si torno a executar el rígid ja es substituiran les noves per les velles?
                #Codi meu
                if i == 0:
                    mask_path = os.path.join(mask_dir_rigid_serial_rigid, "Prova1.png")
                elif i == 1:
                    mask_path = os.path.join(mask_dir_rigid_serial_rigid, "Prova2.png")
                prova = slide_obj.image
                warp_tools.save_img(mask_path, prova)
                #

                features_in_mask_idx = warp_tools.get_xy_inside_mask(xy=img_obj.kp_pos_xy, mask=slide_obj.rigid_reg_mask)
                #Codi meu
                #Codi per guardar la rigid_reg_mask en el directori processed i poder veure-la
                #warp_tools.save_img(os.path.join(img_dir, f'{slide_obj.name}_mask_serial_rigid_warp_tools_kpoints.png'),  slide_obj.rigid_reg_mask)
                #print(f"Mascara de nom rigid_reg_mask que es passa al warp tools per trobar les features dins la bounding box: {slide_obj.rigid_reg_mask}")
                #
    ...
    if registrar.aleady_sorted:
        print(f"Entra al already_sorted")
    ...
    #print(f"Size del registre: {registrar.size}")
    if registrar.size > 2:
    ...
    registrar.finalize(mask_dir_rigid_registration, mask_dir_rigid_serial_rigid)
    #Codi meu: He afegit que li passo a la funció finalize el mask_dir per a que pugui fer el càlcul del IoU (explicat al registration.py, rigid_registrar quan crido la funció register_images)

    if align_to_reference:
        registrar.wiggle_to_ref(transformer)
        #No esntra print(f"Estem aquí")

    if dst_dir is not None:
        #No entra print(f"Entra aqui")
